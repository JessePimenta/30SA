...
class Model_Subscribe extends ORM {
    ...
    public static function notify(Model_Ad $ad)
    {
        ...
        if(isset($ad->cf_model) AND $ad->cf_model) 
           $subscribers->where('cf_model', 'LIKE', '%'.$ad->cf_model.'%');

        if(isset($ad->cf_make) AND $ad->cf_make) 
           $subscribers->where('cf_make', 'LIKE', '%'.$ad->cf_make.'%');

        if(isset($ad->cf_year) AND $ad->cf_year) 
           $subscribers->where('cf_year', 'LIKE', '%'.$ad->cf_year.'%');
