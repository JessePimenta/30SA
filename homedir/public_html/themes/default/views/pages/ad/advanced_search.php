...
<div class="well recomentadion clearfix">
    <h3><?=__('Advanced Parts Search')?></h3>
    <?= FORM::open(Route::url('search'), array('class'=>'form-inline', 'method'=>'GET', 'action'=>''))?>
        <fieldset>

                <div class="form-group">
                <?= FORM::label('advertisement', __('Year/Make/Model or Part Name'), array('class'=>'', 'for'=>'advertisement'))?>
                <div class="control mr-30">
                    <input type="text" id="title" name="title" class="form-control" value="<?=core::get('title')?>" placeholder="<?=__('Year Make & Model or Part Name')?>">
                </div>
                </div>
		...
		<br></br>
                <?if(core::config('advertisement.location') != FALSE AND count($locations) > 1):?>
                    <div class="form-group">
                        ...
                        <div class="control mr-30">
                            <select <?=core::config('general.search_multi_catloc')? 'multiple':NULL?> name="location<?=core::config('general.search_multi_catloc')? '[]':NULL?>" id="location" class="form-control" data-placeholder="<?=__('Where are you?')?>">
                            <?if ( ! core::config('general.search_multi_catloc')) :?>
                ...
                <div class="form-group">
                    ...
                    <div class="control mr-30">
                        <input type="text" id="price-min" name="price-min" class="form-control" value="<?=core::get('price-min')?>" placeholder="<?=__('Price Low')?>">
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <div class="control mr-30">
                        <input type="text" id="price-max" name="price-max" class="form-control" value="<?=core::get('price-max')?>" placeholder="<?=__('Price High')?>">
                    </div>
                </div>
                <?endif?>
