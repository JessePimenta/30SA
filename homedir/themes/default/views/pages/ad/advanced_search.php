...
<div class="well recomentadion clearfix">
    ...
    <?= FORM::open(Route::url('search'), array('class'=>'form-inline', 'method'=>'GET', 'action'=>''))?>
        <fieldset>
                ...
                <?= FORM::label('advertisement', __('Make/Model or Part'), array('class'=>'', 'for'=>'advertisement'))?>
                <div class="control mr-30">
                    <input type="text" id="title" name="title" class="form-control" value="<?=core::get('title')?>" placeholder="<?=__('make/model, part name')?>">
                </div>
                </div>
...
<br></br>
                <?if(core::config('advertisement.location') != FALSE AND count($locations) > 1):?>
                    <div class="form-group">
                        ...
                        <div class="control mr-30">
                            <select <?=core::config('general.search_multi_catloc')? 'multiple':NULL?> name="location<?=core::config('general.search_multi_catloc')? '[]':NULL?>" id="location" class="form-control" data-placeholder="<?=__('Select Your Location')?>">
                            <?if ( ! core::config('general.search_multi_catloc')) :?>
